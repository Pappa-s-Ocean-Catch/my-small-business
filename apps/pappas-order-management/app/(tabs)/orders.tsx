      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            const newOrder = payload.new as Pick<Order, 'id' | 'created_at' | 'order_channel' | 'payment_method' | 'customer_name' | 'scheduled_pickup_at' | 'order_status'>;
            const s = appSettingsRef.current;
            ...
            // Auto-print is owned by app/(drawer)/(tabs)/live-orders.tsx.
            // Keep this legacy screen read-only for realtime refresh/sound to avoid
            // duplicate kitchen tickets when both live-order surfaces are mounted.
          }
          loadOrders();
        }
      )
