...
export async function savePosOrder(
  ...
  try {
    ...
    const { data, error: orderError } = await supabase.rpc('save_pos_order_atomic', {
      p_order_id: null,
      p_order: {
...
export async function updatePosOrder(
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('save_pos_order_atomic', {
      p_order_id: orderId,
      p_order: { ...orderTotalsUpdate, ...normalizedOrderUpdate },
      p_items: updatedItems,
    });
    if (error || !data) throw new Error(error?.message || 'Failed to update POS order.');
    return { data: mapEmbeddedOrder(data as OrderWithEmbeddedItemsRow), error: null };
  } catch (error) {
