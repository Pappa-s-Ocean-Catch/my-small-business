...
test('returns the twelve highest-selling non-cancelled products from today cart items', async () => {
  const modulePath = './todays-top-sellers.js';
  const { rankTodayTopSellers } = await import(modulePath) as {
    rankTodayTopSellers: (rows: SaleRow[]) => Array<{ productId: string; quantitySold: number; orderCount: number }>;
  };
  ...
  assert.deepEqual(rankTodayTopSellers(rows), [
...
test('uses Australia/Melbourne midnight boundaries for today', async () => {
  const modulePath = './todays-top-sellers.js';
