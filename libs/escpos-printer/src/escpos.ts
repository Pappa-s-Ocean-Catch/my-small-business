...
export function encodeTextLine(line: string): Uint8Array {
  return Uint8Array.from([...encodeText(line), LF]);
}

function encodeAlign(align: EscPosAlign = "left"): Uint8Array {
  const value = align === "center" ? 1 : align === "right" ? 2 : 0;
  return Uint8Array.from([ESC, 0x61, value]);
}

function encodeBold(enabled = false): Uint8Array {
  return Uint8Array.from([ESC, 0x45, enabled ? 1 : 0]);
}

function encodeUnderline(enabled = false): Uint8Array {
  return Uint8Array.from([ESC, 0x2d, enabled ? 1 : 0]);
}

function encodeInvert(enabled = false): Uint8Array {
  return Uint8Array.from([GS, 0x42, enabled ? 1 : 0]);
}

function encodeFont(font: EscPosFont = "A"): Uint8Array {
  return Uint8Array.from([ESC, 0x4d, font === "B" ? 1 : 0]);
}

function encodeSize(style?: EscPosTextStyle): Uint8Array {
...
export function buildStyleBytes(style?: EscPosTextStyle): Uint8Array {
  return concatBytes([
...
export function buildTextNode(text: string, style?: EscPosTextStyle, newline = true): EscPosDocumentNode {
  return {
    type: "text",
    text,
    style,
...
export function buildFeedNode(lines = 1): EscPosDocumentNode {
  return {
    type: "feed",
    lines,
  };
...
export function buildCutNode(partial = false): EscPosDocumentNode {
  return {
    type: "cut",
    partial,
  };
...
export function buildRawNode(data: Uint8Array): EscPosDocumentNode {
  return {
    type: "raw",
    data,
  };
...
export function buildImageNode(image: EscPosRasterImage): EscPosDocumentNode {
  return {
    type: "image",
    image,
  };
...
function encodeFeed(lines = 1): Uint8Array {
  const count = Math.max(1, Math.min(255, Math.trunc(lines)));
  return Uint8Array.from([ESC, 0x64, count]);
}

function encodeCut(partial = false): Uint8Array {
  return Uint8Array.from([GS, 0x56, partial ? 1 : 0]);
}

function encodeBitImage24(image: EscPosRasterImage): Uint8Array {
  const width = image.width;
  const height = image.height;
  const rgba = image.data;
  const parts: Uint8Array[] = [Uint8Array.from([ESC, 0x33, 24]), encodeAlign(image.align)];
...
export function encodeImage(image: EscPosRasterImage): Uint8Array {
...
export function encodeDocumentNode(node: EscPosDocumentNode): Uint8Array {
  switch (node.type) {
    case "text":
      return concatBytes([
    ...
    case "raw":
      return node.data;
    default:
      throw new Error(`Unsupported document node ${(node as { type?: string }).type ?? "unknown"}`);
  }
}
...
export function buildTextPrintJob(
  lines: string[],
  options?: { cut?: boolean; initialize?: boolean; style?: EscPosTextStyle }
): Uint8Array {
