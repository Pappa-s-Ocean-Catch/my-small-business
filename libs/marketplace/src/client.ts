  ...
  return {
    ...
    async getHistory(provider, options = {}): Promise<MarketplaceHistoryResult> {
      ...
      if (provider === 'uber_eats') {
        if (mode === 'scheduled') {
          const locationUuid = restaurant(session.cookies);
          const localDate = (date: typeof start) => `${date.year}-${String(date.month).padStart(2, '0')}-${String(date.day).padStart(2, '0')}`;
          const query = `query ordersV2($filters: Orders_OrdersFiltersInput!, $pagination: Orders_OrdersPaginationInput) { ordersV2(filters: $filters pagination: $pagination) { rows { orderId workflowUuid orderUuid salesTotal netPayout requestedAt deliveryTimeLocal courierName fulfillmentType issueType orderStatus orderChannel eater { name isEatsPassSubscriber subscriptionPass } } paginationResult { nextCursor } } }`;
          const body = { operationName: 'ordersV2', variables: { filters: { orderStatusFilter: [], search: '', dateRange: { start: localDate(start), end: localDate(end) }, locationConstraints: { cities: [], countries: [], locationUUIDs: [locationUuid] }, displayCurrencyCode: 'AUD', currentTab: 'scheduledOrders' }, pagination: { limit: 20, cursor: options.cursor || '', nextTable: 'liveOrders' } }, query };
          const payload = await providerJson(transport, 'Uber Eats', 'scheduled history', 'https://merchants.ubereats.com/manager/graphql?op=ordersV2', { method: 'POST', headers: { accept: '*/*', 'content-type': 'application/json', origin: 'https://merchants.ubereats.com', 'x-csrf-token': 'x', Cookie: session.cookies }, body: JSON.stringify(body), cache: 'no-store' });
          const rows = payload?.data?.ordersV2?.rows;
          if (!Array.isArray(rows)) throw new Error('Uber Eats scheduled history request failed');
          return { provider, orders: rows.map((order: any) => ({ orderId: order.orderId || '', workflowUuid: order.workflowUuid || '', orderUuid: order.orderUuid || '', customerName: order.eater?.name || 'Customer', salesTotal: order.salesTotal || '', netPayout: order.netPayout || '', requestedAt: order.deliveryTimeLocal || order.requestedAt || '', courierName: order.courierName || '', fulfillmentType: order.fulfillmentType || '', issueType: order.issueType || order.orderStatus || 'Scheduled', orderChannel: order.orderChannel || '', isSubscriber: !!order.eater?.isEatsPassSubscriber, subscriptionPass: order.eater?.subscriptionPass || '' })).filter((order: any) => order.orderId && order.workflowUuid), nextCursor: payload.data.ordersV2.paginationResult?.nextCursor || null };
        }
