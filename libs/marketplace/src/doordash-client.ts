...
function headers(session: MarketplaceSessionBundle, detail = false) { const ddAttKey = typeof session.providerConfig.ddAttKey === 'string' ? session.providerConfig.ddAttKey : undefined; return { accept: detail ? 'application/json' : '*/*', 'accept-language': 'en-GB', 'content-type': 'application/json', origin: 'https://www.doordash.com', ...(detail ? { 'client-version': 'web version 2.0', 'origin-app': 'merchant_portal' } : {}), ...BROWSER_HEADERS, referer: 'https://www.doordash.com/', 'sec-fetch-site': 'same-site', ...(ddAttKey ? { 'dd-att-key': ddAttKey } : {}), Cookie: session.cookies }; }
...
async function request(transport: MarketplaceTransport, operation: string, url: string, init: RequestInit) { const response = await transport({ url, init }); const payload = await response.json().catch(() => null); if (!response.ok) throw new Error(`DoorDash ${operation} request failed (${response.status})`); return { payload, status: response.status }; }
